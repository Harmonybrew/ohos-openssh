...
cd openssh-10.2p1
./configure \
    --prefix=/opt/openssh-10.2p1-ohos-arm64 \
...
mkdir -p /opt/openssh-10.2p1-ohos-arm64/bin
cp ssh scp sftp ssh-keygen /opt/openssh-10.2p1-ohos-arm64/bin
cd ..

# 进行代码签名
cd /opt/openssh-10.2p1-ohos-arm64
...
cat <<EOF > /opt/openssh-10.2p1-ohos-arm64/licenses.txt
...
# 打包最终产物
cp -r /opt/openssh-10.2p1-ohos-arm64 ./
tar -zcf openssh-10.2p1-ohos-arm64.tar.gz openssh-10.2p1-ohos-arm64
