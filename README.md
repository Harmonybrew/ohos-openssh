...
本项目为 OpenHarmony 平台编译了 openssh，并发布预构建包。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-openssh/releases) 获取。
...
这个项目使用本地编译（native compilation，也可以叫本机编译或原生编译）的做法来编译鸿蒙版 openssh，而不是交叉编译。

需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 openssh 的本地编译。

示例：
```sh
git clone https://github.com/Harmonybrew/ohos-openssh.git
cd ohos-openssh
docker run \
  --rm \
  -it \
